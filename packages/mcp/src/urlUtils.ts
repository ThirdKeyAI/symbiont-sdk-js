 ...
 * and call sites use BARE paths (e.g. `/agents/{id}/execute`). This helper guarantees
 * the final URL contains exactly one `/api/v1` prefix regardless of whether the base
 * URL already ends with `/api/v1` or the endpoint already starts with `/api/v1`.
 */
